...
#---------------------------------------------------
echo ""
echo "Script para Youtube-Dl: Baixar Vídeos em MP4 e Músicas em MP3 (1.2.0)"
echo "====================================================================="
echo ""
...
audio_function(){
	...
	echo "(1) Pasta 'Músicas' do usuário atual"
	echo "(2) Pasta atual do script"
	read LOCATIONM
	#---------------------------------------------------
	if [[ $LOCATIONM -eq 1 ]] ; then
		echo ""
		echo "Verificando a existência da pasta 'Músicas'..."
		#------------------------------------------------
		if [ -e ~/Músicas ] ; then
			EXISTM=1
		else
		...
		if [[ $EXISTM -eq 1 ]] ; then
			cd ~/Músicas
			echo ""
			echo "A pasta 'Músicas' foi selecionada!"
		else
			echo ""
			echo "A pasta 'Músicas' não existe! Deseja criá-la? [S/N]"
			read CREATEM
			CREATEMM="${CREATEM^^}"
			#---------------------------------------------
			if [[ $CREATEMM -eq S ]] ; then
				...
				cd ~/Músicas
				echo ""
				echo "A pasta 'Músicas' foi criada e selecionada!"
			else
				echo ""
				echo "Neste caso, a pasta atual do script será selecionada!"
			fi
