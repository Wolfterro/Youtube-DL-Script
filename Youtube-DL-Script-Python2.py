...
def help():
	print("=============================================")
	print("Youtube-DL Script - Versão %s - Python 2.x" % (version))
	print("=============================================\n")
...
def install_youtube_dl(home_dir):
	...
	try:
		response = urllib2.urlopen(latest)
		data = response.read()
	except Exception:
...
def youtube_dl_options(home_dir):
	...
	user_input = raw_input("Escolha uma das opções acima: ")
...
def get_video_id():
	video_id = raw_input("Insira APENAS a ID do Vídeo ou da Playlist: ")
	if len(video_id) > 11:
		is_playlist = True
	else:
		is_playlist = False
...
if __name__ == "__main__":
	...
	if argc > 1:
		...
		while True:
			print(" ")
			one_more_time = raw_input("Deseja executar o script mais uma vez? [s/N]: ")
